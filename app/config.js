...
const envSchema = z.object({
  ...
  SESSION_MAX_AGE_MS: z.coerce.number().int().min(60_000).max(604_800_000).default(28_800_000),
...
function createConfig({ env = process.env } = {}) {
  ...
  const parsedEnv = envSchema.safeParse({
    ...
    SESSION_MAX_AGE_MS: readNumberEnv(env.SESSION_MAX_AGE_MS, 1000 * 60 * 60 * 8),
    ...
    MSSQL_USER: requireEnv("MSSQL_USER", readStringEnv(env.MSSQL_USER, "")),
    MSSQL_PASSWORD: requireEnv("MSSQL_PASSWORD", env.MSSQL_PASSWORD),
  ...
  const sameSite = normalizeSameSite(values.SESSION_SAME_SITE, "lax");
  ...
  const config = {
    ...
    mssql: {
      ...
      encrypt: readBooleanEnv(values.MSSQL_ENCRYPT, true),
      trustServerCertificate: readBooleanEnv(values.MSSQL_TRUST_SERVER_CERTIFICATE, isTest || !isProduction),
